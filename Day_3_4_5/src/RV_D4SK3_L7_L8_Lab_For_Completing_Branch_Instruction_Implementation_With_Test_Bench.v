...
\TLV
   ...
   |cpu
      @0
         // ---- PC: mux of {reset value, previous branch/jump target, PC + 4 (the "+1")} ----
         $reset = *reset;

         // Step 2 of the lab: use the PREVIOUS $br_tgt_pc when the PREVIOUS instruction
         // was a taken branch (or jump). Reset wins over everything.
      ...
      @1
         ...
         // ---- Dec: immediate (for B-type: a signed 13-bit byte offset, bit 0 always 0) ----
         ...
         // The six branches share the B-type opcode and differ only in funct3.
         ...
         // Anything outside the supported set is illegal. That includes B-type
         // instructions with funct3 = 010 or 011, which are not real branches.
         // Only instructions that were really fetched are checked ($imem_rd_en is
         // 0 during reset and in the very first cycle, when $instr is just zeros).
         $is_supported = $is_add || $is_addi || $is_beq || $is_bne || $is_blt ||
                         $is_bge || $is_bltu || $is_bgeu || $is_jal;
         $illegal_instr = $imem_rd_en && ! $is_supported;
         ...
         // (Branches have no destination register, so they never write.)
         $rf_wr_en = $rd_valid && ($rd != 5'b00000) && ! $illegal_instr;
         $rf_wr_index[4:0] = $rd;
         $rf_wr_data[31:0] = $rf_wr_en ? $result : 32'd0;

         // ---- Branch support ----
         // Step 1 of the lab: the branch target is the PC plus the immediate
         // (the "+" block in the diagram). JAL uses the same target.
         $br_tgt_pc[31:0] = $pc + $imm;

         // The "< >" block: decide whether the branch is taken.
         // Signed compares (BLT/BGE) use the sign-difference trick; unsigned ones compare directly.
         ...
         // The PC mux redirects for a taken branch or a JAL.
         ...
         // Target must lie inside the program (this also catches a negative target,
         // which shows up as a huge unsigned address).
         $br_tgt_out_of_range = $pc_redirect && ($br_tgt_pc[31:2] >= M4_NUM_INSTRS);
         // PC has run past the last instruction (the instruction memory would silently wrap).
         $pc_out_of_range = $imem_rd_en && ($pc[31:2] >= M4_NUM_INSTRS);

         // ---- OR-aggregation of all error conditions into one signal ----
         $error = $illegal_instr || $misaligned_tgt || $br_tgt_out_of_range || $pc_out_of_range;

         // ---- Simulation control (within @1, as in the lab) ----
         // Pass when the final sum is in r10 (read 5 cycles later, once it has settled).
         // Fail as soon as any error condition is raised. The first two cycles are
         // ignored: in cycle 0 the flops between stages have not been loaded yet, so
         // stage @1 sees start-up values, and a failure there would end the
         // simulation immediately (blank waveform).
         *passed = |cpu/xreg[10]>>5$value == (1 + 2 + 3 + 4 + 5 + 6 + 7 + 8 + 9);
         *failed = $error && (*cyc_cnt > 1);
