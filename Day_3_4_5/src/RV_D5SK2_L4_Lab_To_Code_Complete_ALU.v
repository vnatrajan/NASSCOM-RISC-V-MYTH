...
\TLV
   ...
   // /==========================================================\
   // | Lab: Complete Instruction Decode & Complete ALU          |
   // \==========================================================/
   //
   // This file is the deliverable for two MYTH-workshop lab steps:
   //   Step A, Complete Instruction Decode -- finish $is_* for every remaining
   //     RV32I instruction except loads (@1, below), and generate $is_load
   //     from opcode alone (that lab's second instruction).
   //   Step B, Complete ALU -- assign $result for every remaining instruction
   //     listed in the lab (@3, below), including the two comparison ops
   //     (SLT/SLTI).
   // Both sections are marked "THIS LAB" at their start, below.
   //
   // Everything else in this file (the @0 PC mux, the @2/@3 register-file
   // bypass, $taken_br/$valid, and the deferred load write-back at @3, plus the
   // error-handling signals and the halt-loop program padding) is carried
   // forward, unchanged, from the pipeline-timing derivation already done and
   // verified in this same exercise series' branch/hazard-handling lab -- not
   // new work for this step. It's included here (rather than left at the
   // literal, unfixed reference) because the decode/ALU logic below is only
   // testable end-to-end via the full "sum 1 to 9" program, which exercises
   // branches and a load/store round trip; the reference's own version of that
   // surrounding logic has known timing bugs (documented in that earlier
   // writeup) that prevent the program from computing the right answer, so a
   // literal copy would compile but simulate incorrectly. Using the
   // already-verified version instead keeps this deliverable actually correct
   // and SandPiper-clean, while this writeup's explanation focuses on what
   // this lab actually asked for: decode and the ALU.
   //
   // /====================\
   // | Sum 1 to 9 Program |
   // \====================/
      ...
      @0
         ...
         $pc[31:0] = (>>1$reset)     ? 32'd0 :
                     ...
                     (>>3$valid_is_load)  ? (>>3$pc + 32'd4) :
                                            (>>1$pc + 32'd4);
      @1
         // ==== THIS LAB (1/2): Instruction Decode ====
         ...
         // decodes against with a `==?` wildcard match (funct7[5] alone, not the
