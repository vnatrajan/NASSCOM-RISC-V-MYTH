...
\TLV
   ...
   // /==========================================================\
   // | Lab: Redirect Loads (load data from memory to the RF)   |
   // \==========================================================/
   //
   // This file is the deliverable for the "Redirect Loads" lab -- using the
   // ISA to load data from memory into the register file. Three pieces are
   // marked "THIS LAB" at their start, below, all in |cpu's @3 block:
   //   Step A -- Clear $valid in the "shadow" of a load, the same way it's
   //     already cleared in the shadow of a taken branch -- see $valid_is_load
   //     and $valid's own window, just below $taken_br.
   //   Step B -- Select the PC from 3 instructions ago for the load redirect
   //     -- see the @0 PC mux's >>3$valid_is_load term, just below.
   //   Step C -- The deferred write-back that actually lands the loaded data
   //     in the register file once the redirect completes -- $ld_data /
   //     $rf_wr_en / $rf_wr_index / $rf_wr_data, further down @3.
   // Steps A and B are this lab's two explicitly listed instructions; step C
   // is included because a load redirect that never actually writes the
   // loaded value back to $rd would leave the lab's own stated goal -- getting
   // the loaded data into the register file -- unmet, and all three are timed
   // against each other (derived together in the writeup).
   //
   // Everything else in this file (instruction decode, the ALU, the
   // register-file bypass, and $taken_br/branch redirection) is carried
