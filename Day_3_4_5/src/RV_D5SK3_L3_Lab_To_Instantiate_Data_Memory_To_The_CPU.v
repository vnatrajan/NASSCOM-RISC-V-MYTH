...
\TLV
   ...
   // /==========================================================\
   // | Lab: Load Data (instantiate the data memory into the CPU) |
   // \==========================================================/
   //
   // This file is the deliverable for the "Instantiate Data Memory" lab. The
   // lab's own two instructions are marked "THIS LAB" at their start, below,
   // both in |cpu's @3 block plus the macro-instantiation line at the very
   // bottom of the file:
   //   Step 1 -- Uncomment m4+dmem(@4): instantiate the 16-entry, 32-bit-wide
   //     mini data memory (dmem) itself. See the |cpu macro instantiations
   //     near the end of the file.
   //   Step 2 -- Connect the interface signals ($dmem_rd_en, $dmem_wr_en,
   //     $dmem_addr, $dmem_wr_data, and the returned $dmem_rd_data via
   //     $ld_data) using address bits [5:2] of the computed load/store
   //     address, gated on $valid -- see the block right below $jalr_tgt_pc.
   //
   // Both pieces were already necessary groundwork for the PRIOR "Redirect
   // Loads" lab (that lab's SW/LW round trip can't function without dmem
   // actually being instantiated and wired up), so they're already present
   // and working in this file -- carried forward unchanged, not new lines --
   // but weren't that lab's own explicit focus. This lab's writeup is where
   // dmem's interface itself -- the memory macro's shape, the address-bit
   // mapping, and why each connecting signal is gated the way it is -- gets
   // explained in detail.
   //
   // Everything else in this file (instruction decode, the ALU, the
   // register-file bypass, $taken_br/branch redirection, and the load-shadow
   // $valid window/deferred write-back from the "Redirect Loads" lab) is
