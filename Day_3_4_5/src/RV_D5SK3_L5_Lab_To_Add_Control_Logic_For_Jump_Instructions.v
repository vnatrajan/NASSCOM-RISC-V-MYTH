...
\TLV
   ...
   // \==========================================================/
   //
   // This lab asks for four things, all marked "THIS LAB" at their start,
   // below (all in |cpu's @1/@3 blocks):
   //   1. Define $is_jump (JAL or JALR), and, like $taken_br, create invalid
   //      cycles -- see $is_jump's definition at @1, and the note at
   //      $valid_taken_br / $valid_jalr (@3) explaining how JAL's and
   //      JALR's invalid-cycle windows already implement this, kept as two
   //      separate windows rather than one merged $is_jump-gated window.
   //   2. Compute $jalr_tgt_pc (SRC1 + IMM) -- see $jalr_tgt_pc at @3.
   //   3. Select correct $pc for JAL (>>3$br_tgt_pc) and JALR
   //      ($jalr_tgt_pc) -- see the @0 PC mux.
   //   4. Save -- this delivered file, plus its accompanying writeup.
   //
   // Unlike the last several labs, where the asked-for logic was ALREADY
   // present from earlier bug-fixing work and this lab's job was mostly to
   // frame and verify it, items 2 and 3 here genuinely were already fully
   // built and correct in the prior "Redirect Loads" lab's own bug-D fix
   // (JALR's redirect needed a dedicated, validity-gated path from day one,
   // since it can't share a target-computation term with branches the way
   // JAL can). Item 1 ($is_jump itself, as a named signal) is this lab's own
   // new addition -- not because the invalidation behavior it describes was
   // missing, but because the lab specifically asks for it to exist and be
   // used, and it gives $illegal_instr and this writeup a single, clean name
   // for "either kind of unconditional jump" instead of always spelling out
   // "$is_jal || $is_jalr".
   //
   // Everything else in this file (instruction decode, the ALU, the
   // register-file bypass, branch redirection, the load-shadow $valid
   // window/deferred write-back, and the dmem interface wiring) is carried
   // forward, unchanged, from earlier steps in this same exercise series,
   // already verified end to end -- not new work for this lab.
   //
   // /====================\
   // | Sum 1 to 9 Program |
   // \====================/
   ...
   // (carried forward from the "Load/Store in Program" lab, not this lab's own scope)
   ...
   |cpu
      @0
         $reset = *reset;
         // ==== THIS LAB, Instruction 3: select correct $pc for JAL and JALR ====
         // JALR gets first priority, through its own dedicated term: (>>3$valid_jalr) ?
         // (>>3$jalr_tgt_pc). JAL does NOT get a separate term of its own here -- $taken_br
         // (defined at @3, below) already resolves unconditionally to 1'b1 for every $is_j_instr
         // (JAL's own format class), so JAL rides the SAME (>>3$valid_taken_br) ? (>>3$br_tgt_pc)
         // term already used for ordinary taken branches. This matches the lab's own framing
         // exactly -- "JAL (>>3$br_tgt_pc) and JALR ($jalr_tgt_pc)" -- selecting each from its
         // own already-correct target computation. (carried forward from the "Redirect Loads"
         // lab, not this lab's own scope): the >>3$valid_is_load term below is the load redirect,
         // same 3-cycle-wide shadow, same underlying mechanism, just a different target
         // ("continue from here" rather than a computed jump/branch destination).
      ...
      @1
         ...
         // ==== THIS LAB, Instruction 1: define $is_jump (JAL or JALR) ====
         // A single umbrella flag for "this is an unconditional jump, of either kind" -- used
         // below in $illegal_instr (so the legal-opcode check doesn't need to spell out
         // "$is_jal || $is_jalr" every time), and referenced in this lab's writeup when
         // discussing jump shadow cycles / control hazard handling as a unit. It is NOT used to
         // gate the $valid invalidation window or the @0 PC mux directly -- see the note at
         // $valid_taken_br / $valid_jalr (@3) for why JAL and JALR keep separate windows there
         // despite both being "jumps" at the decode level.
      ...
      @3
         ...
         // (carried forward from the "Redirect Loads" lab, not this lab's own scope, except
         // where noted below for $valid_jalr)
         ...
         // ==== THIS LAB, Instruction 1 (continued): jump shadow cycles for JAL and JALR ====
         // The lab asks for $is_jump to "create invalid cycles" the same way $taken_br does.
         // That already happens here, split across the two lines above/below rather than merged
         // into one $is_jump-gated term: JAL is a $is_j_instr, which $taken_br (@3, above) already
         // forces to 1'b1 unconditionally, so JAL's own invalid-cycle window falls out of
         // $valid_taken_br for free -- no separate JAL-specific term is needed. JALR gets its own
         // dedicated $valid_jalr term here instead of also folding into $valid_taken_br, because
         // JAL and JALR need DIFFERENT redirect targets at the @0 PC mux ($br_tgt_pc vs
         // $jalr_tgt_pc, Instruction 2/3 below) -- $valid's 3-slot window (just above) already
         // ORs in both !(...$valid_taken_br) and !(...$valid_jalr) across all 3 slots, so the
         // net effect is exactly "any jump (JAL or JALR) creates 3 invalid cycles after it",
         // i.e. $is_jump's invalidation behavior, just implemented as two co-operating windows
         // rather than one signal named $valid_jump.
         ...
         // ==== THIS LAB, Instruction 2: compute $jalr_tgt_pc (SRC1 + IMM) ====
         // JALR's target is register-indirect plus an immediate offset (unlike JAL, whose
         // target is PC-relative via $br_tgt_pc above) -- $src1_value is the base register
         // (already bypass-corrected, from the register-file forwarding logic above), $imm is
         // the I-type immediate extracted at @1 from the same instruction. This is exactly the
         // "SRC1 + IMM" the lab asks for.
         $jalr_tgt_pc[31:0] = $src1_value + $imm;

         // (carried forward from the "Load Data" lab, not this lab's own scope)
         ...
         // (carried forward from the "Redirect Loads" lab, not this lab's own scope)
         ...
         // Uses $is_jump (THIS LAB) in place of spelling out "$is_jal || $is_jalr" -- purely a
         // naming simplification, same two opcodes covered either way.
         $illegal_instr = $valid && !($is_lui || $is_auipc || $is_jump ||
